...
const ProductPage = ({title}) => {
    ...
    useEffect(() => {
      if (!isLoaded) {
        const fetchData = async () => {
          ...
          const productsData = await AuthFetchService.fetch('/api/products/','GET',null,jwt);
    ...
    const addToCart = async (productId) => {
        ...
        try {
          setIsLoaded(false);
          const existingCartItem = await AuthFetchService.fetch(`/api/cart-items/cart/${activeCart.id}/product/${productId}`, 'GET', null, jwt);
          if (existingCartItem) {
              const updatedCartItem = {...existingCartItem, quantity: existingCartItem.quantity + 1};
              await AuthFetchService.fetch(`/api/cart-items/${existingCartItem.id}`, "PUT", updatedCartItem, jwt);
          } else {
              const product = await AuthFetchService.fetch(`/api/products/${productId}`, 'GET', null, jwt);
              const newCartItem = { cartId: activeCart.id, quantity: 1, product:  product};
              await AuthFetchService.fetch('/api/cart-items', 'POST', newCartItem, jwt);
          }
        //   setShowToast(true);
        //   setTimeout(() => {
        //       setShowToast(false);
                ...
                <div className="product-items">
                    {products.map((product, index) => (
                        <div key={index} className="product-item-card">
                            <img className="product-icon" src={product.imageUrl} height="63" width="63" alt="product" />
